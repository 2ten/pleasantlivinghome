...
function create_entry(path, lastmod) {
    return `<url>
    <loc>${new URL(path, 'https://www.website.com').href}</loc>
...
export async function GET({ fetch, setHeaders, request }) {
    ...
    const pages = await client.get({ 
        filters: [
          prismic.filter.any('document.type',["page","project","furniture","collection","press"]),
        ],
        orderings: {
    ...
    const pageEntries = pages.results.map((page) => create_entry(page.url,page.last_publication_date));
